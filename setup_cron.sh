#!/bin/bash
# Setup cron jobs for crash monitoring at 10 AM and 3 PM

echo "Setting up cron jobs for crash probability monitoring..."
echo ""
echo "This will run checks at:"
echo "  - 10:00 AM (before market open)"
echo "  - 3:00 PM (before market close)"
echo ""
echo "Emails will be sent to:"
echo "  - tao.zhang.1977@gmail.com"
echo "  - gao.yuan.77@gmail.com"
echo ""
echo "Only when crash probability exceeds 60%"
echo ""

# Get the current crontab
crontab -l > /tmp/current_cron 2>/dev/null || touch /tmp/current_cron

# Remove any existing crash monitor entries
grep -v "proj_stock_monitor/daily_check.sh" /tmp/current_cron > /tmp/new_cron

# Add new cron jobs
# 10 AM every weekday (Monday-Friday)
echo "0 10 * * 1-5 /home/tom2zhang/workspace/proj_stock_monitor/daily_check.sh" >> /tmp/new_cron

# 3 PM every weekday (Monday-Friday)
